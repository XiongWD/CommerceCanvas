...
const OUT = 'artifacts/frontend/g2-f1';
...
async function captureViewport(vw, vh) {
  ...
  // 1. idle（初始）
  await page.screenshot({ path: path.join(OUT, 'idle-1440x900.png'), animations: 'disabled' });
  shots.push({ file: 'idle-1440x900.png', kind: 'idle' });
  ...
  // 2. running-analysis：等到轨迹有几条 + 有阶段在跑
  await page.waitForTimeout(3000);
  await page.screenshot({ path: path.join(OUT, 'running-analysis.png'), animations: 'disabled' });
  shots.push({ file: 'running-analysis.png', kind: 'running' });
  ...
  await page.waitForTimeout(800);
  await page.screenshot({ path: path.join(OUT, 'recovered.png'), animations: 'disabled' });
  shots.push({ file: 'recovered.png', kind: 'recovered' });
