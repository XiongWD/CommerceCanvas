/**
 * F3：竞品套图分析页面（从原 App.tsx 抽出）。
 *
 * - Live Intelligence 状态来自 AppShell 提供的 LiveContext（不再各自挂载模拟器）。
 * - DemoControls / AmbientStatus / GlobalRail / PersistentTaskBar 已上提到 AppShell。
 * - 保留：投影层、统一导航、sessionKey 重置、ContextSidebar/InspectorPanel key、
...
import { AnalysisTrace, MilestoneReveal } from '@/features/live-intelligence';
import { selectRecipeCompleteness } from '@/features/live-intelligence/state/live-intelligence-selectors';
import {
  projectCompetitorAnalysis,
  type RecipeFieldKey,
} from '@/features/competitor-analysis/state/competitor-analysis-projection';
 ...
 * F3：从 AppShell 经 LiveContext 获取 live API。
 */
export function CompetitorAnalysisPage() {
  ...
  // —— Recipe→来源事件（§三）——
  const handleNavigateRecipe = useCallback(
    (field: RecipeFieldKey) => {
  ...
  const handleSelectSellingPoint = useCallback(
    (spId: string) => {
      if (spId === '') {
